...
function SoundManager(b,a){
	...
	this.defaultOptions = {autoLoad:false,
		stream:true,
		autoPlay:false,
		onid3:null,
		onload:null,
		...
		pan:0,
		volume:100};
		this.flash9Options={isMovieStar:null,
			usePeakData:false,
			useWaveformData:false,
			useEQData:false,
			onbufferchange:null,
			ondataerror:null};
			this.movieStarOptions={onmetadata:null,
				useVideo:false,bufferTime:null};
				var f=null;
				var e=this;
				this.version=null;
				...
				var g=function(h){return document.getElementById(h)};
				...
				this._setVersionInfo=function(){
					if(e.flashVersion!=8&&e.flashVersion!=9){
						...
						e.flashVersion=e._defaultFlashVersion}e.version=e.versionNumber+(e.flashVersion==9?" (AS3/Flash 9)":" (AS2/Flash 8)");
						if(e.flashVersion>8){
						...
						if(e.flashVersion>8&&e.useMovieStar){
							...
							e.features.movieStar=false}e.filePattern=e.filePatterns[(e.flashVersion!=8?"flash9":"flash8")];
							...
							this._waitingforEI=false;this._initPending=false;this._tryInitOnFocus=(this.isSafari&&typeof document.hasFocus=="undefined");
							...
							this.strings={notReady:"Not loaded yet - wait for soundManager.onload() before calling sound-related methods",appXHTML:"soundManager._createMovie(): appendChild/innerHTML set failed. May be app/xhtml+xml DOM-related.",localFail:'soundManager: Loading this page from local/network file system (not over HTTP?) Flash security likely restricting JS-Flash access. Consider adding current URL to "trusted locations" in the Flash player security settings manager at '+d+", or simply serve this content over HTTP.",waitFocus:"soundManager: Special case: Waiting for focus-related event..",waitImpatient:"soundManager: Getting impatient, still waiting for Flash.. ;
							)",waitForever:"soundManager: Waiting indefinitely for Flash...",needFunction:"soundManager.onready(): Function object expected"};
							this.supported=function(){
								return (e._didInit&&!e._disabled)};
								this.getMovie=function(h){return e.isIE?window[h]:(e.isSafari?g(h)||document[h]:g(h))};
								this.loadFromXML=function(h){try{e.o._loadFromXML(h)}catch(i){e._failSafely();
									return true}};
									this.createSound=function(i){var l="soundManager.createSound(): ";
									if(!e._didInit){
										throw e._complain(l+e.strings.notReady,arguments.callee.caller)
									}if(arguments.length==2){
										i={id:arguments[0],url:arguments[1]}}var j=e._mergeObjects(i);
										var h=j;
										...
										return e.sounds[h.id]};
										this.createVideo = function(h){if(arguments.length==2){
										...
										this.destroySound=function(j,h){
											...
											if(!h){e.sounds[j].destruct()}delete e.sounds[j]};
														...
														if(!e._didInit){
															...
															if(!e._idCheck(h)){
																if(typeof i!="Object"){
																	...
																	i.id=h;
																	e.createSound(i)}else{return false}
																}e.sounds[h].play(i)};
																...
																this.setPosition=function(h,i){
																	...
																	this.stop=function(h){
																		...
																		this.stopAll=function(){
																			...
																			this.pause=function(h){if(!e._idCheck(h)){
																				return false
																			}e.sounds[h].pause()};
																				...
																				for(var h=e.soundIDs.length;h--;)
																					{
																						...
																						this.resume=function(h){if(!e._idCheck(h)){
																							return false
																						}e.sounds[h].resume()};
																						...
																						this.setPan=function(h,i){
																							if(!e._idCheck(h)){
																								return false}e.sounds[h].setPan(i)};
																								this.setVolume=function(i,h){
																									if(!e._idCheck(i)){
																										return false}e.sounds[i].setVolume(h)
																									};
																										...
																										if(typeof h!="string"){
																											h=null}if(!h){
																												for(var j=e.soundIDs.length;j--;)
																													{
																														e.sounds[e.soundIDs[j]].mute()}e.muted=true
																													}else{
																														if(!e._idCheck(h)){
																															return false
																														}
																													...
																													if(typeof h!="string"){
																														...
																														for(var j=e.soundIDs.length;j--;)
																															...
																															e.muted=false}
																																...
																																if(!e._idCheck(h)){
																																	return false}e.sounds[h].unmute()
																																}
																															};
																															this.unmuteAll=function(){e.unmute()
																															};
																															...
																															this.disable=function(j){if(typeof j=="undefined"){
																																j=false
																															}if(e._disabled){
																																return false
																															}e._disabled=true;
																															for(var h=e.soundIDs.length;h--;)
																																{
																																	e._disableObject(e.sounds[e.soundIDs[h]])}e.initComplete(j)};
																																		...
																																		this.getSoundById=function(i,j){
																																			...
																																			this.onload=function(){
																																				...
																																				this._complain=function(i,k){
																																					var j="Error: ";
																																					if(!k){
																																						return new Error(j+i)}var m=new Error("");
																																						var n=null;
																																						if(m.stack){
																																							try{
																																								var o="@";
